...
#    agent2 and agentcontroller2 and run the go tests
# 2- If passed, first step it will run an agent-controller on the master machine
# 3- It will create 3 more machines on ms1 and install the agent2 (using ays) on the
...
data = {
    'instance.param.name': 'agent2_master',
...
vmMaster = j.atyourservice.new(name='node.ms1', instance='agent2_master', args=data)
vmMaster.install()
...
ubuntu.network.setHostname('agent2-master')
...
# if this is not a clean machine make sure to stop agentcontroller2

cl.run('ays stop -n agentcontroller2')

# running tests.
with shell_env(GOROOT='/opt/go', GOPATH='/opt/go_workspace', PATH='PATH=$PATH:/opt/go/bin'):
    cl.run('go get -u -t -f github.com/Jumpscale/agent2')
    cl.run('go get -u -t -f github.com/Jumpscale/agentcontroller2')

    cl.run('go test -v github.com/Jumpscale/agent2/tests')
...
controller = j.atyourservice.new(name='agentcontroller2', parent=vmMaster, args=data)
controller.consume('node', vmMaster.instance)
controller.install(deps=True)

client = j.atyourservice.new(name='agentcontroller2_client', parent=vmMaster, args=data)
client.consume('node', vmMaster.instance)
client.install(deps=True)

# start the controller service
cl.run('ays start -n agentcontroller2')
...
for i in range(3):
    nid = i + 1
    data = {
        'instance.param.name': 'agent2_%s' % nid,
        'instance.param.memsize': 1,
        'instance.param.ssdsize': 10,
        'instance.param.imagename': 'ubuntu.14.04.x64',
        'instance.paran.ssh.shell': '/bin/bash -l -c',
    ...
    vmAgent = j.atyourservice.new(name='node.ms1', instance='agent2_%d' % nid, args=data)
    vmAgent.install()

    agentCl = vmMaster.actions._getSSHClient(vmAgent)

    ubuntu = j.ssh.ubuntu.get(agentCl)
    ubuntu.network.setHostname('agent2-%d' % nid)
...
for i in range(3):
    ...
    assert osinfo['hostname'] == 'agent2-%s' % nid, 'Invalid os response from agent'
"""
